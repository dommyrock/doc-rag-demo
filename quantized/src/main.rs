...
use lib::token_output_stream::TokenOutputStream;
...
fn main() -> anyhow::Result<()> {
    ...
    let device = lib::device(args.cpu)?;

    println!("Current device is {:?},   is cude: {:?}",device, device.is_cuda());
    //let device = Device::new_cuda(0)?; // Error: no cuda implementation for rms-norm
