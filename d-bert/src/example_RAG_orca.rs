...
//https://github.com/scrippt-tech/orca/blob/81b24c92bb80496547b6db0fc6e0d810cdf9aaf7/orca-core/src/llm/bert.rs#L111
    ...
    qdrant
        .insert_many(&collection, embeddings.to_vec2()?, pdf_records)
        .await?;

    // Use prompt to query Qdrant
    let query_embedding = bert.generate_embedding(prompt!(args.prompt)).await?;
    let result = qdrant
        .search(&collection, query_embedding.to_vec()?.clone(), 1, None)
        .await?;
